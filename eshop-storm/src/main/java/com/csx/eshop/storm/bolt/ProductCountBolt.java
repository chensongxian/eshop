...
import com.csx.eshop.storm.http.HttpClientUtils;
import com.csx.eshop.storm.zk.ZooKeeperSession;
				...
				try {
					...
					for(Map.Entry<Long, Long> productCountEntry : productCountList) {
						if(productCountEntry.getValue() > 10 * avgCount) {
							...
							String distributeNginxURL = "http://192.168.0.172/hot?productId=" + productCountEntry.getKey();
							HttpClientUtils.sendGetRequest(distributeNginxURL);

							// 将缓存热点，那个商品对应的完整的缓存数据，发送请求到缓存服务去获取，反向推送到所有的后端应用nginx服务器上去
							String cacheServiceURL = "http://192.168.0.198:8080/getProductInfo?productId=" + productCountEntry.getKey();
							String response = HttpClientUtils.sendGetRequest(cacheServiceURL);

							String[] appNginxURLs = new String[]{
									"http://192.168.0.170/hot?productId=" + productCountEntry.getKey() + "&productInfo=" + response,
									"http://192.168.0.171/hot?productId=" + productCountEntry.getKey() + "&productInfo=" + response
							};
						...
						if(hotProductIdList.size()>0){
						 ...
						 for(Long productId:lastTimeHotProductIdList){
							if(!hotProductIdList.contains(productId)){
								...
								// 说明上一个热点消失
								// 发送一个http请求取消热点缓存标识
								String url = "http://192.168.0.172/cancel_hot?productId=" + productId;
